    ...
    p.render = function render() {
        ...
        if (state.status.visible && state.status.active) {
            ...
            if (state.items.listOpen) {
                this.listContainer = $("<div id='" + this.elem.id + "_listBoxWrapper' class='listBoxContainer " + state.style.styleToApply + ((state.style.additionalStyleToApply) ? ' ' + state.style.additionalStyleToApply : '') + "'></div>");
    ...
    */
    p.getLimits = function (cropToParent, el, appView, bodyEl) {
        var limits = {
            ...
            viewportRect = _getViewportRect(bodyEl),
    ...
    function _getViewportRect(bodyEl) {
        // visualViewport is the correct modern way to get viewport dimensions, other method does not work for zoomed virtual viewport (#A&P 749840)
        if (window.visualViewport) {
            var vv = window.visualViewport;            
            var scale = Math.max(1, vv.scale);

            return _toAbsoluteRect({
                top: vv.offsetTop * scale,
                left: vv.offsetLeft * scale,
                bottom: (vv.offsetTop + vv.height) * scale,
                right: (vv.offsetLeft + vv.width) * scale,
                width: vv.width * scale,
                height: vv.height * scale
            });
        }
        return _toAbsoluteRect(bodyEl.get(0).getBoundingClientRect());
    }

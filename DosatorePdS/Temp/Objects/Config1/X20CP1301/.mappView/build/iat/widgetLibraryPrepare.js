(function () {
    ...
    var widgetLibraryPrepare = {
        ...
        createSchemaset: function createSchemaset(library, options) {
            let f = utils.formatter(options.format);
            var namespace = '';
            switch (options.type) {
                case 'content':
            ...
            var xml = '<?xml version="1.0" encoding="utf-8"?>' + f.lbr() + '<SchemaCatalog xmlns="http://schemas.microsoft.com/xsd/catalog">' + f.lbr();

            for (var entry in library) {
                if (typeof library[entry] === 'string') {
                    xml += f.tab(1) + '<Schema href="' + library[entry] + '" targetNamespace="' + namespace + '" />' + f.lbr();
                } else if (typeof library[entry] === 'object' && library[entry].href !== undefined) {
                    xml += f.tab(1) + '<Schema href="' + library[entry].href + '" targetNamespace="' + library[entry].namespace + '" />' + f.lbr();
                }
            }

            xml += '</SchemaCatalog>' + f.LF;

            return xml;
        },

        runAsHTMLIndex: function runAsHTMLIndex(widgetInfo, options) {

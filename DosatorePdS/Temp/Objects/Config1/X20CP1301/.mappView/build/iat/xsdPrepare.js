(function () {
    ...
    /**
     * Creates widget xsd files for use in automation studio. 
     */
    var xsdPrepare = {
...
`<xs:schema targetNamespace="http://www.br-automation.com/iat2015/styles/engineering/v1"
        ...
        runWidgetStyleDefinition: function runWidgetStyleDefinition(widgetInfo, options) {
            ...
            if (widgetInfo.styleproperties !== undefined && widgetInfo.styleproperties.StyleProperty.length > 0) {
                ...
                for (i = 0; i < widgetInfo.styleproperties.StyleProperty.length; i += 1) {
                    ...
                    if (prop.hide !== true && prop.hide !== 'true' && prop.not_styleable !== 'true') {
                        attributeCnt++;
    ...
    function _attrType(type) {
        return ` type="types:${type}"`;
    }

    function _attrUse(use) {
        return ` use="${use}"`;
    }

    function _attrName(name) {
        return ` name="${name}"`;
    }
    ...
    function createWidgetXsd(widgetInfo, DataTypes, Properties) {
        ...
        var basePropNames = Properties.getBasePropNames();
        var widgetPropNames = widgetInfo.properties.map(prop => prop.name);

        basePropNames.forEach(basePropName => {
            // if property is defined in BaseWidget but not in widget => restrict use in xsd
            if (!widgetPropNames.includes(basePropName)) {
                const baseProp = Properties.getBaseProp(basePropName);
                if (baseProp.hideable !== false) {
                    attributeRestrictions += tab(3) + '<xs:attribute' + _attrName(basePropName) + _attrUse('prohibited') + _attrType(baseProp.type) + ' />' + lbr();
                }
            }
        });

        widgetInfo.properties.forEach(function (prop) {
            hide = String(prop.hide) === 'true';
            attrName = _attrName(prop.name);
            attrType = _attrType(prop.type);
            if (DataTypes.isArrayType(prop.type)) {
                ...
                if (prop.owner === 'brease.core.BaseWidget' && prop.defaultValue !== undefined) {
                    ...
                    if (!baseProp) {
                        throw new Error(`unknown base property ${prop.name} in xsdPrepare.createWidgetXsd`);
                    } else {
                        attrUse = _attrUse(baseProp.use);
                        attrType = _attrType(baseProp.type);
                    }
